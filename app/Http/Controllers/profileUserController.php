...
class profileUserController extends Controller {
	...
	public function indexAdmin($user) {
		$activeUser = auth()->user();

		if (isset($activeUser) && !is_null($activeUser) && auth()->user()->role == 1) {
			$editUser = DB::table('users')
				->where('username', $user)->get()->first();

			if (isset($editUser) && !is_null($editUser)) {
				return view('profile.editarPerfil')->with('user', $editUser)->with('ruteF', 'modifiedUserByAdmin');
			} else {
				return "No se encuentra el usuario en BD, o acaba de ser eliminado<br>
					<a href='javascript:history.back()'>Go Back</a>";
			}
	...
	public function modifyByAdmin(Request $request) {
		$datosForm = $request;
...
//
			...
			return view('profile.editarPerfil')->with('success', 'Modify user Ok')->with('user', $editUser)->with('ruteF', 'modifiedUserByAdmin');
		} else {
			return "Unahutorized, you must be logged <br>
					<a href='javascript:history.back()'>Go Back</a>";
		}
	...
	public function modify(Request $request) {
		...
		if ($user = auth()->user() && ($datosForm["userNam"] == auth()->user()->userName)) {
